...
export const EventStepper = memo(
  ({ currentStep, labels, stepStatus, currentStepRevealed }: EventStepperProps) => {
  ...
  return (
    <div className='flex items-center w-full'>
      {STEP_ORDER.map((_, index) => {
        ...
        /** 정답 공개로 채워지는 경우 → 왼쪽→오른쪽 애니메이션 적용 (이미 풀었던 스텝으로 돌아갔을 때는 애니메이션 재생 안 함) */
        const isAlreadyCompleted = status !== 'default';
        const leftFilledByReveal =
          step > 1 && step === currentIndex + 1 && !!currentStepRevealed && !isAlreadyCompleted;
            ...
            <div className='flex items-center w-full'>
              ...
              <div
                className={clsx(
                  ...
                  circleBg,
                  step === currentIndex && step > 1 && status === 'default' && 'animate-stepper-circle-immediate',
                )}
              >
                {step}
              </div>
                ...
                {rightFillColor && (
                  <div
                    ...
                    className={clsx(
                      ...
                      rightFillColor,
                      rightFilled
                        ? step === currentIndex && status === 'default'
                          ? 'animate-stepper-fill'
                          : 'scale-x-100'
                        : 'scale-x-0',
                    )}
