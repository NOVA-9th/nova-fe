...
const SideBar = () => {
  return (
    <nav className='w-80 px-4 pb-4 flex flex-col justify-between'>
      <section className='flex flex-col gap-1 cursor-pointer'>
        <SideTabItemCustom peak={true} icon={MessagesSquare} className='flex items-center h-16'>
        ...
        <SideTabItemCustom icon={ChartGantt}>
          <div className='px-0.5 flex-1'>
            <h3>트렌드</h3>
            <p className='typo-callout-base'>IT 트렌드 키워드 분석 및 시각화</p>
          </div>
        </SideTabItemCustom>
        <SideTabItemCustom icon={Bookmark}>
          <div className='px-0.5 flex-1'>
            <h3>저장함</h3>
      ...
      <section className='flex items-center justify-between h-14.5 px-2'>
        ...
        <button type='button' className='text-optional'>
          <LogOut size={16} />
        </button>
      </section>
    </nav>
